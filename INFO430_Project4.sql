...
GO
    ...
    --Views
        ...
        --create a view for the top 1000 passenger who have done the most trips on cruises partitioned by their cabin room
        CREATE VIEW passMostTrips AS
        SELECT TOP 1000 P.PassengerID, P.PassengerFname, P.PassengerLname, C.CabinID, C.CabinName, COUNT(T.TripID) AS numTrips, RANK() OVER (PARTITION BY C.CabinID ORDER BY COUNT(T.TripID) DESC) AS RankOfTrips
        FROM PASSENGER P 
        JOIN BOOKING B ON P.PassengerID = B.PassengerID
        JOIN TRIP T ON B.TripID = T.TripID
        JOIN BOOK_CABIN BC ON BC.BookingID = B.BookingID
        JOIN CABIN C ON BC.CabinID = C.CabinID
        GROUP BY P.PassengerID, P.PassengerFname, P.PassengerLname, C.CabinID, C.CabinName
        ORDER BY RankOfTrips ASC

select * from numMembershipOnTrip
